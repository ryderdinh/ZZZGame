...
namespace AiToolbox {
[CustomPropertyDrawer(typeof(DalleParameters))]
public class DalleParametersDrawer : PropertyDrawer {
    private float _height;

    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
        label.text = "DALL·E Parameters";
        label.tooltip = "Settings for the backend of the OpenAI DALL·E API.";
        ...
        // Question mark button
        {
            ...
            if (GUI.Button(buttonRect, icon, EditorStyles.label)) {
                Application.OpenURL("https://ai-toolbox.dustyroom.com/dalle-runtime");
            }
        }
        ...
        if (property.isExpanded) {
            ...
            // API Key
            {
                var apiKeyProperty = property.FindPropertyRelative(nameof(DalleParameters.apiKey));
                ...
                _height += rect.height + EditorGUIUtility.standardVerticalSpacing;
                var encryptionProperty = property.FindPropertyRelative(nameof(DalleParameters.apiKeyEncryption));
                ...
                var passwordProperty = property.FindPropertyRelative(nameof(DalleParameters.apiKeyEncryptionPassword));
            ...
            // API Key encryption
            {
                ...
                if (encryptionProperty.enumValueIndex == (int)ApiKeyEncryption.None &&
                    !string.IsNullOrEmpty(property.FindPropertyRelative(nameof(DalleParameters.apiKey)).stringValue)) {
                ...
                if (EditorGUI.EndChangeCheck()) {
                    ...
                    var passwordProperty =
                        property.FindPropertyRelative(nameof(DalleParameters.apiKeyEncryptionPassword));
                    var isEncrypted = encryptionProperty.enumValueIndex == (int)ApiKeyEncryption.LocallyEncrypted;
                    if (isEncrypted) {
                ...
                if (showRemoteConfigField) {
                    var remoteConfigKeyProperty =
                        property.FindPropertyRelative(nameof(DalleParameters.apiKeyRemoteConfigKey));
            ...
            // Model
            {
                var modelProperty = property.FindPropertyRelative(nameof(DalleParameters.model));
                var rect = new Rect(position.x, position.y + _height, width, EditorGUIUtility.singleLineHeight);
            ...
            // Size
            {
                var sizeProperty = property.FindPropertyRelative(nameof(DalleParameters.size));
                var rect = new Rect(position.x, position.y + _height, width, EditorGUIUtility.singleLineHeight);
            ...
            // Quality
            {
                var qualityProperty = property.FindPropertyRelative(nameof(DalleParameters.quality));
                var rect = new Rect(position.x, position.y + _height, width, EditorGUIUtility.singleLineHeight);
                EditorGUI.PropertyField(rect, qualityProperty);
                _height += EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing;
            }
            ...
            // Timeout
            {
                var timeoutProperty = property.FindPropertyRelative(nameof(DalleParameters.timeout));
            ...
            // Throttle
            {
                var throttleProperty = property.FindPropertyRelative(nameof(DalleParameters.throttle));
